...
describe("privacy-aware VAYRA backups", () => {
  it("excludes LUMA activity unless the user opts in", async () => {
    ...
    const activity = JSON.stringify({ schemaVersion: 1, profileId: "default", queue: [], resumes: [] });
    values.set("vayra.luma.v1.default", activity);
    ...
    expect(complete.data["vayra.luma.v1.default"]).toBe(activity);
  ...
  it("preserves current LUMA activity when restoring a backup that excludes it", async () => {
    values.set("harbor.settings", JSON.stringify({ uiLanguage: "en" }));
    ...
    const backup: Backup = {
      ...
      data: { "harbor.settings": JSON.stringify({ uiLanguage: "fr" }) },
      includesLocalActivity: false,
    };

    await applyBackup(backup);
    expect(values.get("harbor.settings")).toBe(JSON.stringify({ uiLanguage: "fr" }));
  ...
  it("restores and labels opted-in LUMA activity", async () => {
    ...
    const text = JSON.stringify({
      ...
      app: "test",
      exportedAt: "",
      data: { "harbor.settings": "{}", "vayra.luma.v1.default": activity },
    });
    ...
    expect(values.get("vayra.luma.v1.default")).toBe(activity);
  });
});
