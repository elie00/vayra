    ...
    <main className="fixed inset-0 z-[120] flex items-center justify-center overflow-hidden bg-black px-6">
      ...
      >
        ...
        <h2 id="no-sources-title" className="mt-3 text-[24px] font-semibold leading-tight text-ink">
          {t("No streaming sources yet")}
        </h2>
        <p className="mt-3 text-[14px] leading-relaxed text-ink-muted">
          {t("VAYRA needs at least one streaming source before it can play {title}. Pick one of the options below to get set up.", { title })}
        </p>
        <ul className="mt-3 space-y-1.5 text-[13.5px] leading-relaxed text-ink-muted">
          <li>{t("· Install a stream addon (Torrentio, Comet, MediaFusion).")}</li>
          <li>{t("· Add a debrid key (TorBox, Real-Debrid, AllDebrid, Premiumize, Debrid-Link).")}</li>
        </ul>
