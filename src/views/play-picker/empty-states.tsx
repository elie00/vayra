...
export function NoSourcesState({
  ...
  const tip = isAnime
    ? t("Anime sources are usually richer through Torrentio's anime config or AIOStreams. Make sure one is installed in Stremio.")
    : isWeb
      ? t("On the web, VAYRA can only reach addons that allow browser access (Torrentio, TorBox, Cinemeta). For unreleased titles, no source typically exists yet.")
      : t("Try signing in to Stremio so VAYRA can use your addon collection. Older or foreign titles often need Torrentio + a debrid addon to find anything.");
  return (
    <div className="rounded-[24px] border border-edge-soft/70 bg-canvas/80 px-9 py-11">
      <div className="flex flex-col items-center gap-5 text-center">
        ...
        <h2 className="font-display text-[28px] leading-tight text-ink">
          {addonCount === 1
            ? t("VAYRA queried {count} addon and got nothing back", { count: addonCount })
            : t("VAYRA queried {count} addons and got nothing back", { count: addonCount })}
        </h2>
        ...
        <p className="text-[10.5px] font-mono uppercase tracking-[0.18em] text-ink-subtle/70">
          {t("Tried IDs:")} {streamIds.slice(0, 3).join(" · ")}
          {streamIds.length > 3 ? ` · ${t("+{count} more", { count: streamIds.length - 3 })}` : ""}
        </p>
      </div>
    </div>
  );
...
export function FilteredOutState({
  ...
  return (
    <div className="relative overflow-hidden rounded-[24px] border border-edge-soft/70 bg-canvas/80 px-9 py-11">
      <div className="flex flex-col items-center gap-5 text-center">
        <p className="text-[10.5px] font-bold uppercase tracking-[0.42em] text-ink-subtle">
          {t("{count} sources · 0 made it through", { count: rawCount })}
        </p>
        ...
        <p className="max-w-lg text-[14px] leading-relaxed text-ink-muted">
          {t(
            "VAYRA blocks suspicious files and mismatched releases by default. For older shows and unusual titles this is sometimes too tight.",
          )}
        </p>
        {groups.length > 0 && (
    ...
    <div className="relative overflow-hidden rounded-[24px] border border-edge-soft/60 bg-canvas/60">
      ...
      <div className="relative flex flex-col items-center gap-4 px-10 py-14 text-center">
        ...
        <p className="max-w-md text-[14px] leading-relaxed text-ink-muted">
          {t(
            "No clean release has surfaced yet. This may be too new. VAYRA's filters dropped everything that came back as wrong-movie noise.",
          )}
        </p>
