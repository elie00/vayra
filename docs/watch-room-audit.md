...
| **CAST-2** | 8 | Bouton cast gaté par le **moteur** (mpv=activé, html5/exo=désactivé) → cast indisponible pour le live/IPTV forcé sur html5 | ✅ RECLASSÉ — **comportement intentionnel**, pas un défaut : le bouton l'énonce lui-même (« Casting comes with the mpv backend », `cast-button.tsx:22`) et seul mpv expose `chromecast=true`. Le cast est une fonctionnalité du backend mpv. L'élargir (cast depuis html5/exo) est une **décision produit** qui exige d'abord de valider que le pipeline cast sait sourcer hors-mpv + du matériel de test — **pas une correction à l'aveugle** |
...
- **ARCHIVE-ROOM-GAP** : ✅ **DÉCIDÉ + IMPLÉMENTÉ** (migration `20260715120000`, non encore déployée). Décision : archiver **gèle vraiment** le group-VARA — `vara_rooms` persiste désormais `group_id`, et `cira_archive_group` **ferme** les rooms encore ouvertes du groupe. Une room ne porte aucune donnée durable, donc la fermer ne détruit rien de persistant ; l'ordre de verrous groupe→room est conservé partout (pas de deadlock). Les rooms sans groupe sont intactes. Test d'éviction ajouté à `20_group_archive.sql`.
